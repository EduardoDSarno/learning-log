...
{
	// check first
	if(tolower(id[0]) != 'a') return 0; 
	if (strlen(id)!= 9) return 0; // 8 digits + a
...
void return_second_word(const char input[], char output[])
{
	...
	else // else used to work as a return in cases of empty strings
	{
		// skip inital space
		while (isspace(input[i])) {
			i++;
...
{
	// I did not like having to check for every end of string every if
	// will study ways to simplify it
	int count = 0;
	int i = 0;
	// a word is the combination of not space w/ space
	while (input[i] != '\0') {
		// while is space we continue/ignore
...
void return_nth_word(const char input[], char output[], int n)
{
	...
	else
	{
		for(int i = 0; input[i] != '\0'; i++){
			// if curr char is not space and (previous char is space or i == 0)
			// the i == 0 comes before the input [ i-1] so that it does nto go out of bound
